...
@if_test_k8s
def test_k8s_ingress_0(client, kube_hosts):
    ...
    create_service_ingress_validate(ingresses, services, port, namespace,
                                    hostheader1="foo.bar.com", hostheader2="foo.bar.com",
                                    path1="/service3.html", path2="/name.html")

    delete_ingress(ingress_name, namespace)
    teardown_ns(namespace)

@if_test_k8s
def test_k8s_ingress_21(client, kube_hosts):
    # This method tests an ingress with host, paths specified and two services
