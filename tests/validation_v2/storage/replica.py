...
Commands:
  ...
  load-volume -f <file name>'''
    ...
    # storage things
    sgroup = OptionGroup(
        parser, "Connection options", "these define addresses, ports,"
                                      " etc needed for connectivity")
...
class Connection:
    ...
    def invoke(self, terms):
        ...
        if terms[0] == "capabilities":
            ...
            for r in resp.replicas.item:
                rv["replicas"].append((r.host, r.port,
                                       ("", "Online")[r.online] +
                                       ("", "Splicing")[r.splicing]))
            return rv
        elif terms[0] == "status":
    ...
    def send(self, buf):
        self.print_dbg(
            "data: " + binascii.hexlify(buf[0:4]) + " " + binascii.hexlify(
                buf[4:]))
        self.s_.send(buf)
    ...
    def recv(self, size):
        ...
        while len(rx) < size:
            # check whether the very front of the data stream
            #  contains a failure
            (processed, resp) = self.extract_resp(rx)
            if not resp.io_result.success:
                return rx
    ...
    def extract_buf(self, buf):
        ...
        size = hdr & 0x1fffff
        self.print_dbg(
            "header: type=" + str((hdr >> 21) & 31) + " msg_size=" + str(
                size) + \
...
if __name__ == "__main__":
    ...
    # as this requires explicit invoke_xxxx() calls and
    # extra arguments
    ...
    if terms[0] == "capabilities":
        ...
        for (off, size) in rv:
            if options.xml:
                print('  <range offset="' + str(off) + '" size="' + str(
                    size) + '"/>')
            else:
        ...
        print("Got status:")
        print("  status: " + block_pb2.Response.ReplicationInfo.Status.Name(
            rv["status"]))
    elif terms[0] == "replicas":
