...
def shutdown_one(name):
    ...
    if rc != (None, None):
        sys.stderr.write("error: " + name + " terminated with " + str(rc) + "\n")
        rv = rc
    del daemons[name]
    return rv
...
def poll_replica(rport):
    ...
    # (that happens with manual runs when the human forgets to run "rm -f big*")
    retries = 0
    while True:
        if retries > 150:
...
def create_replica(number):
    ...
    daemons[replica_name] = subprocess.Popen(['./apps/replica/replica', '-n', big_name,
...
parser.add_argument("-r", "--replicas", 
                    ...
                    "note: an extra final replica is always created", type=int, default=2)
parser.add_argument("-k", "--kill", 
                    help="kill random replicas abruptly and attempt to relaunch",
                    type=int, default=0)
parser.add_argument("-kc", "--killcase", help="choose case for terminating a replica", 
                    type=str, default="basic",
                    choices =["basic","under_load","pre_negotiating_st","insync_st"])
parser.add_argument("-ls", "--livesnap", help="toggle live snapshots [off by default]", 
                    dest='livesnap', action='store_true', default=False)
parser.add_argument("--rsize", help="declare size of replica [20G by default]", type=str,
                    default='20G')
parser.add_argument("--rslab", help="declare size of slabs [1G by default]", type=str,
                    default='1G')
parser.add_argument("-w", "--web", help="use the web and declare mb size of download"
                    "[5 by default]", type=int, choices=[5,10,20,50,100], default=5)
parser.add_argument("-dd", "--diskdump", help="use dd and declare count of 4096b blocks"
                    " like 1 for 1 block", type=int, choices=[1,10,100,500,1000,5000])
...
daemons["client"] = subprocess.Popen(['./apps/controller/controller']+client_rports+['-m', str(cport)])
...
for i in range(0, 10):
    ...
    nbd = subprocess.Popen(['sudo', 'nbd-client', '-c', nbd_dev], stdout=subprocess.PIPE)

    # Find the disconnected device (it does not print the PID)
    if nbd.stdout.readline():
        log("Warning: Skipping disconnected " + nbd_dev)
        continue # try next

    log("Using " + nbd_dev)
    nbd = subprocess.Popen(['sudo', 'nbd-client', '-b', '4096', 'localhost', nbd_dev, 
                            '-N', 'disk', '-n'])
    ...
    for i in range(1, 30):
        ...
        real_sleep(1)
    if not got_size:
        log("Warning: Skipping offline " + nbd_dev)
    ...
    rc = cmd.wait()
    if rc != 0:
        log("Warning: Skipping unformattable " + nbd_dev)
        os.system("sudo kill %s" % (nbd.pid))
        continue # try next
...
if args.diskdump:
    cmd = subprocess.Popen(['dd', 'if=/dev/zero', 'of=testfile', 'bs=4096',
                            'count='+str(args.diskdump), 'conv=fdatasync', 'oflag=direct'])
else:
    cmd = subprocess.Popen(['sudo', 'wget', 
                            'http://www.web4host.net/'+str(args.web)+'MB.zip'])
...
cli_rv = None
if rv == 0:
    cli_rv = invoke_cli(["connect", "localhost", str(rport[replica_count+1])], cport)
    ...
    while True:
        ...
        if rval and len(rval) == 2:
            ...
            if not have_splicing:
                log("Controller is connected! " + str(len(rval["replicas"])) + " replicas are online.")
                break
...
log('Total elapsed time for this run: ' + str(elapsed_time.seconds)+'.'+str(elapsed_time.microseconds/1000) + ' seconds.')
