...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeService:

    testname = "TestRancherComposeService"

    def test_rancher_compose_service(self, super_client, client,
        ...
        service, env = create_env_and_svc(self.testname, client, launch_config,
                                          scale)
...
class TestRancherComposeServicesPortAndLinkOptions:

    testname = "TestRancherComposeServicesPortAndLinkOptions"
...
class TestRancherComposeLBservice:
    ...
    def test_rancher_compose_lbservice(self, super_client, client,
                                       rancher_compose_container):
        ...
        env, service, lb_service = create_env_with_svc_and_lb(
            self.testname, client, service_scale, lb_scale, port)
...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeLBServiceInternal:

    testname = "TestRancherComposeLBServiceInternal"

    def test_rancher_compose_lbservice_internal(self, super_client, client,
                                                rancher_compose_container):
        ...
        validate_internal_lb(super_client, rancher_lb_service,
                             [rancher_service, rancher_service1],
                             host, con_port, port)
        # Check that port in the host where LB Agent is
        # running is not accessible
...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeServiceLinks:

    testname = "TestRancherComposeServiceLinks"

    def test_rancher_compose_service_links(self, super_client, client,
                                           rancher_compose_container):
        ...
        env, service, consumed_service = create_env_with_2_svc(
            self.testname, client, service_scale, consumed_service_scale, port)
...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeDnsServices:

    testname = "TestRancherComposeDnsServices"

    def test_rancher_compose_dns_services(self, super_client, client,
                                          rancher_compose_container):
        port = "7902"

        rancher_compose_dns_services(self.testname, super_client, client, port,
                                     rancher_compose_container)


@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeDnsServicesCrossStack:

    testname = "TestRancherComposeDnsServicesCrossStack"

    def test_rancher_compose_dns_services_cross_stack(
            self, super_client, client, rancher_compose_container):
        port = "7903"
...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeExternalServices:

    testname = "TestRancherComposeExternalServices"

    def test_rancher_compose_external_services(self, super_client, client,
                                               rancher_compose_container):
        ...
        env, service, ext_service, con_list = create_env_with_ext_svc(
            self.testname, client, service_scale, port)
...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeLBServiceHostRouting:

    testname = "TestRancherComposeLBServiceHostRouting"

    def test_rancher_compose_lbservice_host_routing(self, super_client, client,
                                                    rancher_compose_container):
        port1 = "7906"
        port2 = "7907"
        ...
        env, services, lb_service = \
            create_env_with_multiple_svc_and_lb(
                self.testname, client, service_scale, lb_scale,
                [port1, port2], service_count)
...
@pytest.mark.P0
@pytest.mark.RancherCompose
class TestRancherComposeExternalServicesHostname:

    testname = "TestRancherComposeExternalServicesHostname"

    def test_rancher_compose_external_services_hostname(
            self, super_client, client, rancher_compose_container):
        ...
        env, service, ext_service, con_list = create_env_with_ext_svc(
            self.testname, client, service_scale, port, True)
...
def rancher_compose_dns_services(testname, super_client, client, port,
                                 rancher_compose_container,
                                 cross_linking=False):
