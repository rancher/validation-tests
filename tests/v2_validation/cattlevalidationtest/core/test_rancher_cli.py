...
def test_cli_restart_container(client, rancher_cli_container):
    ...
    container = client.list_container(name="test_cont", removed_null=True).data
...
def cli_delete_service(client, compose_directory,
    ...
    if service.id in cli_response:
        assert True
    mystack = client.list_stack(name=stack_name).data
...
def test_cli_env_list(client, rancher_cli_container):
    ...
    envlist = client.list_project().data
    found = False
    for env in envlist:
...
def cli_inspect_stack(client, compose_directory,
    ...
    stacklist = client.list_stack(name=stack_name).data
...
def test_cli_inspect_host(client, rancher_cli_container):
    ...
    hostlist = client.list_host().data
    print(hostlist)
...
def test_cli_volume_list(client, rancher_cli_container):
    ...
    vol_list = client.list_volume().data
...
def cli_volume_create_remove(client, compose_directory,
    ...
    vol_names_list = []
    for vol in vol_list:
        vol_names_list.append(vol.get('name'))
...
def test_cli_env_create_rm_cattle(admin_client, rancher_cli_container):
    ...
    envlist = admin_client.list_project().data
    for env in envlist:
        if envid == env.id:
            assert False
...
def test_cli_list_system_services(client, rancher_cli_container):
    ...
    stack_name = random_str().replace("-", "")
    services_list = client.list_service(system=True).data
