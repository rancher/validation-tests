...
def test_ldap_create_new_env_add_group_owner(admin_client):
    ...
    project = main_client.by_id('project', project.id)
    project_member = project.projectMembers().data[1]
...
def test_ldap_group_search_domain(admin_client):
    ...
    groups_searched = main_client.list_identity(name=duplicate_name_group).data
...
def test_ldap_change_user_to_admin(admin_client):
    ...
    u2_client = create_ldap_client(username=ldap_user2,
                                   password=ldap_pass2)
    assert u2_client.list_openldapconfig().data is not None
...
def test_ldap_required_to_specific_user(admin_client, request):
    ...
    main_client = create_ldap_client(username=ldap_main_user,
                                     password=ldap_main_pass)
    main_identity = main_client.list_identity(name=ldap_main_user).data[0]

    allowed_identities = []
    allowed_identities.append(main_identity)
    user2_identity = ADMIN_LDAP_CLIENT.list_identity(name=ldap_user2).data[0]
...
def test_ldap_required_to_specific_group(admin_client, request):
    ...
    group_identity = ADMIN_LDAP_CLIENT.list_identity(name=group).data[0]
